...
if ($_POST) {
	if (check_input($_POST, array('api_key', 'action')) == false) {
		$result = array('status' => false, 'data' => array('msg' => 'Request does not match'));
	} else {
		$user = $model->db_query($db, "*", "users", "BINARY api_key = '".mysqli_real_escape_string($db, $_POST['api_key'])."'");
		if ($user['count'] <> 1) {
			$result = array('status' => false, 'data' => array('msg' => 'Incorrect API Key'));
		} else {
			if ($_POST['action'] == 'order') {
				if (check_input($_POST, array('service', 'data', 'quantity')) == false) {
					...
					if ($service['count'] == 0) {
						$result = array('status' => false, 'data' => array('msg' => 'Service not found'));
					} else {
						...
						if ($provider['count'] == 0) {
							$result = array('status' => false, 'data' => array('msg' => 'Service not available'));
						} elseif ($_POST['quantity'] < $service['rows']['min']) {
							$result = array('status' => false, 'data' => array('msg' => 'Number of messages does not match'));
						} elseif ($_POST['quantity'] > $service['rows']['max']) {
							$result = array('status' => false, 'data' => array('msg' => 'Number of messages does not match'));
						} elseif ($user['rows']['balance'] < $total_price) {
							$result = array('status' => false, 'data' => array('msg' => 'Balance is not enough'));
						} else {
							$result_api = false;
							$curl = '';
							$provider_order_id = '3';
							...
							if ($result_api == false) {
								...
								$model->db_insert($db, "balance_logs", array('user_id' => $user['rows']['id'], 'type' => 'minus', 'amount' => $total_price, 'note' => 'Make an Order melalui API. Order ID: '.$insert.'.', 'created_at' => date('Y-m-d H:i:s')));
				...
				if (check_input($_POST, array('id')) == false) {
					...
					if ($order['count'] == 0) {
						$result = array('status' => false, 'data' => array('msg' => 'Order not found'));
					} else {
